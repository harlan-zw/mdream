...
pub fn split_markdown(markdown: &str, opts: &SplitterOptions) -> Vec<MarkdownChunk> {
  assert!(
    opts.chunk_overlap < opts.chunk_size,
  ...
  for i in 0..line_count {
    ...
    let is_frontmatter = frontmatter_end_idx >= 0 && (i == 0 || i == frontmatter_end_idx as usize);
      ...
      if let Some((level, header_text)) = header_match {
        ...
        if opts.headers_to_split_on.contains(&tag_id) {
          ...
          if seen_split_headers & bit != 0 {
            flush_chunk(
              &mut chunks,
              &mut last_chunk_end_position,
              &mut last_split_position,
              &mut line_number,
              &mut current_chunk_code_language,
              &header_hierarchy,
              line_pos,
              false,
              opts.strip_headers,
    ...
    if !opts.return_each_line {
      ...
      if current_chunk_size > opts.chunk_size {
        ...
        let final_split =
          if split_position == -1 || (split_position as usize) <= last_chunk_end_position {
            line_end
          } else {
