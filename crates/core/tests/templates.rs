...
mod wikipedia {
  ...
  fn produces_nonempty_output() {
    ...
    assert!(
      !md.trim().is_empty(),
      "wikipedia output should not be empty"
    );
  }
  ...
  fn contains_links() {
    ...
    assert!(
      md.contains("]("),
      "wikipedia output should contain markdown links"
    );
  }
  ...
  fn contains_tables() {
    ...
    assert!(
      md.contains('|'),
      "wikipedia output should contain table pipes"
    );
  }
...
mod github {
  ...
  fn contains_code_blocks() {
    ...
    assert!(
      md.contains("```"),
      "github output should contain code blocks"
    );
  }
...
mod edge_cases {
  ...
  fn noscript_content_not_rendered() {
    let html =
      "<p>Visible</p><noscript><iframe src=\"analytics\"></iframe></noscript><p>Also visible</p>";
    let md = convert(html);
