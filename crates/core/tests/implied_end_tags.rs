...
fn unclosed_li_followed_by_li_are_siblings() {
  ...
  assert_eq!(
    convert("<ol><li>one<li>two<li>three</ol>"),
    "1. one\n2. two\n3. three"
  );
}
...
fn nested_anchor_closes_the_open_one() {
  ...
  assert_eq!(
    convert("<a href=/1>one<a href=/2>two</a>"),
    "[one](/1) [two](/2)"
  );
  // Intervening inline formatting is closed along with the anchor.
  assert_eq!(
    convert("<a href=/1>one<b>bold<a href=/2>two</a>"),
    "[one**bold**](/1) [two](/2)"
  );
}
...
fn well_formed_and_cross_block_anchors_unchanged() {
  assert_eq!(
    convert("<a href=/1>one</a><a href=/2>two</a>"),
    "[one](/1) [two](/2)"
  );
  // A closed anchor in another block is not affected.
  assert_eq!(
    convert("<div><a href=/1>one</div><a href=/2>two</a>"),
    "[one](/1)\n\n[two](/2)"
  );
}
...
fn streaming_split_inside_trigger_tag_matches_whole() {
  ...
  for (chunks, whole) in [
    ...
    (
      vec!["<dl><dt>Coffee<d", "d>Hot drink</dl>"],
      "<dl><dt>Coffee<dd>Hot drink</dl>",
    ),
  ] {
