...
// benchmark.ts - Compare performance of Downstream vs Turndown
...
const config = {
  ...
  // Maximum file size to test with Turndown (in KB)
...
function generateReport(results: BenchmarkResult[]): string {
  let report = '# Downstream vs Turndown Benchmark\n\n'
  ...
  // Check if Downstream outperforms Turndown in any test
  const downstreamFaster = results.some(r =>
  ...
  report += `- **Performance**: Downstream ${downstreamFaster
    ? `outperforms Turndown in conversion speed for ${
      determinePerformanceDetails(results, 'time')}`
    : 'has comparable speed to Turndown'}\n`

  // Check if Downstream uses less memory than Turndown in any test
  const downstreamMemoryEfficient = results.some(r =>
    r.library === 'downstream'
    && r.success
    && r.memory !== undefined
    && results.some(t =>
  ...
  report += `- **Memory Usage**: Downstream ${downstreamMemoryEfficient
    ? `uses less memory than Turndown for ${
      determinePerformanceDetails(results, 'memory')}`
    : 'has comparable memory usage to Turndown'}\n`
  ...
  report += '\n## Recommendations\n\n'
  report += `- For LLM workflows processing large HTML documents, ${
    downstreamMemoryEfficient
      ? 'Downstream provides substantial memory efficiency benefits\n'
      : 'both libraries perform adequately, with Downstream offering streaming capabilities\n'}`

  report += `- ${downstreamFaster
    ? 'Downstream is recommended for performance-critical applications\n'
    : 'Choose based on feature set rather than performance alone\n'}`

  report += `- For optimal streaming performance, configure chunk size to ${optimalChunkSize} bytes\n`
  ...
  // Add downstream vs turndown comparisons
  for (const testFile of testFiles) {
  ...
  // Check if Downstream and Turndown outputs are equivalent
  const librariesEquivalent = results.every(r =>
    !(r.library === 'downstream'
      && !r.streaming
      && r.equivalence
...
function determinePerformanceDetails(results: BenchmarkResult[], metric: 'time' | 'memory'): string {
  ...
  // Check where Downstream outperforms Turndown
  const smallBetter = [...smallTests].some(testFile =>
    results.some(r =>
      r.library === 'downstream'
      && r.success
      && r[metric] !== undefined
  ...
  const mediumBetter = [...mediumTests].some(testFile =>
    results.some(r =>
      r.library === 'downstream'
      && r.success
      && r[metric] !== undefined
  ...
  const largeBetter = [...largeTests].some(testFile =>
    results.some(r =>
      r.library === 'downstream'
      && r.success
      && r[metric] !== undefined
  ...
  const categories = []
  if (largeBetter)
    categories.push('large documents')
  if (mediumBetter)
    categories.push('medium-sized documents')
  if (smallBetter)
    categories.push('small documents')
...
async function main(): Promise<void> {
  console.log('HTML to Markdown Benchmark: Downstream vs Turndown')
  console.log('=================================================')
  ...
  for (const testFile of testFiles) {
    ...
    // Read the HTML file
    const html = await fs.readFile(testFilePath, 'utf8')
