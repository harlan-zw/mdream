...
describe.each(engines)('plain text output $name', (engineConfig) => {
  ...
  it('falls back from image alt text to title and src', async () => {
    ...
    expect(htmlToMarkdown('<img src="https://example.com/#/route?utm_source=test&width=10">', {
      clean: { urls: true },
      engine,
      format: 'text',
    })).toBe('https://example.com/#/route?utm_source=test&width=10')
    expect(htmlToMarkdown('<img src="image.png">', {
      clean: null as any,
      engine,
      format: 'text',
    })).toBe('image.png')
