...
/**
 * Guard against passing hook-based (array) plugins to the Rust engine. The
 * declarative engine only understands the config shape; custom hook plugins
 * live in `@mdream/js`. Shared by every entry point that maps user options.
 */
export function assertNoHookPlugins(options: Partial<MdreamOptions>): void {
...
/**
 * Map user-facing {@link MdreamOptions} into the normalized shape the Rust
 * engine (NAPI on Node, WASM on edge/browser) understands. This is the single
 * source of truth for `minimal`/`isolateMain`/`tailwind`/`filter`/`frontmatter`
 * → `plugins` normalization; every entry point must funnel through it so the
 * Node and edge builds behave identically.
 */
