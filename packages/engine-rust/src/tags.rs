...
fn is_inside_table_cell(ctx: &HandlerContext) -> bool {
    ctx.node.depth_map[TAG_TD as usize] > 0
}
...
fn enter_strong(ctx: &HandlerContext) -> Option<Cow<'static, str>> {
    if ctx.node.depth_map[TAG_B as usize] > 1 {
...
fn enter_emphasis(ctx: &HandlerContext) -> Option<Cow<'static, str>> {
    if ctx.node.depth_map[TAG_I as usize] > 1 {
        return Some(Cow::Borrowed(""));
    }
...
fn enter_heading(ctx: &HandlerContext) -> Option<Cow<'static, str>> {
    ...
    if ctx.node.depth_map[TAG_A as usize] > 0 {
...
static TAG_HANDLERS: LazyLock<[Option<TagHandler>; MAX_TAG_ID]> = LazyLock::new(|| {
    ...
    table[TAG_BLOCKQUOTE as usize] = Some(TagHandler {
        enter: Some(|ctx| {
            let depth = std::cmp::max(1, ctx.node.depth_map[TAG_BLOCKQUOTE as usize]);
            let mut prefix = "> ".repeat(depth as usize);
            if ctx.node.depth_map[TAG_LI as usize] > 0 {
                let indent = "  ".repeat(ctx.node.depth_map[TAG_LI as usize] as usize);
                prefix = format!("\n{}{}", indent, prefix);
            }
    ...
    table[TAG_CODE as usize] = Some(TagHandler {
        enter: Some(|ctx| {
            if ctx.node.depth_map[TAG_PRE as usize] > 0 {
    ...
    table[TAG_LI as usize] = Some(TagHandler {
        enter: Some(|ctx| {
            ...
            let ul_depth = ctx.node.depth_map[TAG_UL as usize];
            let ol_depth = ctx.node.depth_map[TAG_OL as usize];
            let depth = if ul_depth + ol_depth > 0 { (ul_depth + ol_depth - 1) as usize } else { 0 };
            let is_ordered = ctx.node.depth_map[TAG_OL as usize] > 0 && ctx.ancestors.last().map(|p| p.tag_id == Some(TAG_OL)).unwrap_or(false);
    ...
    table[TAG_TR as usize] = Some(TagHandler {
        ...
        exit: Some(|ctx| {
            if is_inside_table_cell(ctx) || ctx.node.depth_map[TAG_TABLE as usize] > 1 {
    ...
    table[TAG_TH as usize] = Some(TagHandler {
        enter: Some(|ctx| {
            if ctx.node.depth_map[TAG_TABLE as usize] > 1 { return Some(Cow::Borrowed("<th>")); }
    ...
    table[TAG_TD as usize] = Some(TagHandler {
        enter: Some(|ctx| {
            if ctx.node.depth_map[TAG_TABLE as usize] > 1 { return Some(Cow::Borrowed("<td>")); }
            if ctx.node.index == 0 { Some(Cow::Borrowed("")) } else { Some(Cow::Borrowed(" | ")) }
        }),
        exit: Some(|ctx| {
            if ctx.node.depth_map[TAG_TABLE as usize] > 1 { return Some(Cow::Borrowed("</td>")); }
    ...
    table[TAG_CENTER as usize] = Some(TagHandler {
        enter: Some(|ctx| if ctx.node.depth_map[TAG_TABLE as usize] > 1 { Some(Cow::Borrowed("<center>")) } else { None }),
        exit: Some(|ctx| if ctx.node.depth_map[TAG_TABLE as usize] > 1 { Some(Cow::Borrowed("</center>")) } else { None }),
        spacing: Some(NO_SPACING),
        ..Default::default()
    });
    ...
    // Grouped handlers
    for id in [TAG_FOOTER, TAG_FORM, TAG_COL, TAG_EMBED, TAG_PARAM, TAG_SOURCE, TAG_TRACK] {
    ...
    table[TAG_FIGCAPTION as usize] = Some(TagHandler::default());
